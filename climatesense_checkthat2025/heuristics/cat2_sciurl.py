...
def load_domain_files(
    subdomains_file="repo_subdomains.csv",
    sci_mags_file="science_mags_domains.csv",
    sci_news_file="news_outlets_domains.csv",
):
    """Load domain files into variables."""
    subdomains = pd.read_csv(subdomains_file)["domain"].values
    sci_mags_domains = pd.read_csv(sci_mags_file)["domain"].values
    sci_news_domains = pd.read_csv(sci_news_file)["domain"].values
    return subdomains, sci_mags_domains, sci_news_domains
...
def url2domain(url, retry=False, resolve=False):
    ...
    if resolve:
        if "bit.ly" in url or "doi.org" in url:
            try:  # try to resolve bit.ly and doi.org links, if fails continue
                s = requests.Session()
                s.headers = {
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36 Edg/98.0.1108.55"  # noqa: E501
                }
    ...
    if len(domain_parts) >= 2:
        ...
        if retry:
            try:  # try to resolve faulty link, if fails continue (e.g. http://gov.uk has only gov.uk suffix but no domain -> faulty, resolving to www.gov.uk -> domain = www)  # noqa: E501
                s = requests.Session()
                s.headers = {
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36 Edg/98.0.1108.55"  # noqa: E501
                }
...
def prepare_urls(tweets):
    ...
    tweets_w_url = (
        tweets_w_url.groupby(by=cols, dropna=False)
        .agg(
            {
...
def annotate_sci_crossref_subdomains(domains, subdomains):
    matches = []
...
def annotate_sci_mag_domains(domains, sci_mags_domains):
    matches = []
...
def annotate_sci_news_domains(domains, urls, sci_news_domains):
    matches = []
...
def annotate_tweets(tweets, subdomains, sci_mags_domains, sci_news_domains):
    ...
    tweets_w_url["sci_subdomain"] = tweets_w_url["subdomain_domain_tlds"].progress_apply(
        partial(annotate_sci_crossref_subdomains, subdomains=subdomains)
    )
    ...
    tweets_w_url["sci_mag_domain"] = tweets_w_url["subdomain_domain_tlds"].progress_apply(
        partial(annotate_sci_mag_domains, sci_mags_domains=sci_mags_domains)
    )
    ...
    tweets_w_url["sci_news_domain"] = tweets_w_url[["subdomain_domain_tlds", "processed_urls"]].progress_apply(
        lambda x: annotate_sci_news_domains(x[0], x[1], sci_news_domains), axis=1
    )
