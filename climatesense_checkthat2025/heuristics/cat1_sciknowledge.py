...
def load_scientific_terms(wiki_sci_terms_path="wiki_sci_terms.txt", sc_methods_path="sc_methods.txt"):
    with open(wiki_sci_terms_path, "r") as f:
        wiki_sci_terms = [line.strip() for line in f]

    with open(sc_methods_path, "r") as f:
    ...
    # 500 false scientific words are collected then taken away from the scientific terms list, by a process of using the sciterm heuristic and looking at the  # noqa: E501
    # most frequent words collected
    false_scientific_words_list = [
        "haven",
        "practice",
        "fans",
        "call",
    ...
    scientific_terms = [
        ...
        if (term not in text_speak_wordlist) and (term not in false_scientific_words_list)
    ]
...
def contains_arg_relation(tweet_sentence):
    for pred in predicates:
        if re.match(r".*\s(" + pred + r")\s.{2,}", tweet_sentence) is not None:
            return pred
    return ""


# HEURISTIC 2: CONTAINS SCIENTIFIC TERM
def contains_scientific_term(tweet_sentence, scientific_terms, one_gram_sciterm_only=True):
    ...
    # and a bad positive match ("stop .."-> also matches "top"+" "
        ...
        for sciterm in scientific_terms:
            if (
                (" " + sciterm + " " in tweet_sentence)
                or (sciterm + " " in tweet_sentence)
                or (" " + sciterm in tweet_sentence)
            ):
                sciterms.append(sciterm)
        if len(sciterms) > 0:
...
# HEURISTIC 3: IS CLAIM (PATTERN-MATCHING: NOUN + PRED + (NOUN OR ADJ) where PRED = a predicate from the list of predicates)  # noqa: E501
def is_claim(tweet):
    tweet = tweet.lower()
    ...
    if pred != "":
        ...
        for sent in sentences:
            ...
            if " " + pred + " " in sent:
                ...
                # Condition = what's before the predicate IS a noun AND IS NOT one of the following: personal pronoun, possessive pronoun, person including fictional  # noqa: E501
                if (
                    "PRP" not in tags_before
                    and "PRP$" not in tags_before
                    and "PERSON" not in ents_before
                    and "NOUN" in poss_before
                ):
                    # Same condition for what's after the predicate
                    if (
                        "PRP" not in tags_after
                        and "PRP$" not in tags_after
                        and "PERSON" not in ents_after
                        and ("NOUN" or "ADJ" in poss_after)
                    ):
                        if "?" in sent:
...
# COMPOUND HEURISTICS
def is_claim_with_sciterm(tweet, scientific_terms):
    return is_claim(tweet)[0] and contains_scientific_term(tweet, scientific_terms, one_gram_sciterm_only=True)[0]


def annotate_tweets(tweets):
    scientific_terms = load_scientific_terms()
    res = tweets["text"].progress_apply(lambda tweet: is_claim_with_sciterm(tweet, scientific_terms))
    tweets["is_cat1"] = res
